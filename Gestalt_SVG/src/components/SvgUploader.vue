<template>
    <v-card class="fill-height">
        ...
        <v-file-input v-model="file" prepend-icon="mdi-paperclip" label="Select Svg File" density="compact" accept=".svg"
        ...
        <!-- 在现有按钮旁边添加 -->
        <div class="button-group" v-if="isTracking">
            <v-btn 
                color="primary" 
                class="mt-4" 
                @click="filterAndAnalyze" 
            ...
            <v-btn 
                color="error" 
                class="mt-4 ml-2" 
                @click="store.commit('CLEAR_SELECTED_NODES')"
            >
                清除选择
            </v-btn>
        </div>
...
// 添加过滤并分析的函数
const filterAndAnalyze = async () => {
    if (!file.value || selectedNodeIds.value.length === 0) return;
    
    try {
        // 先过滤SVG
        const response = await axios.post('http://localhost:5000/filter_by_ids', {
            filename: file.value.name,
            selectedElements: selectedElements.value,
            selectedIds: selectedNodeIds.value  // 使用 store 中的选中节点
        });
