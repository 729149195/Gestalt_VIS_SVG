<template>
    <div ref="parentContainer" class="combined_chart_container">
        ...
        <!-- 添加矩阵可视化容器 -->
        <div class="matrix_visualization_container">
            <div ref="containmentMatrixContainer" class="matrix_container">
                <h3>包含矩阵 (Containment Matrix)</h3>
            </div>
            <div ref="adjacencyMatrixContainer" class="matrix_container">
...
const containmentMatrixContainer = ref(null);
const adjacencyMatrixContainer = ref(null);
...
let containmentMatrixData; // 包含矩阵数据
let adjacencyMatrixData; // 邻接矩阵数据
...
const generateAnalysis = (dataMapping, dataEquivalentWeights) => {
    ...
    // 适配新的数据结构 - dataMapping现在包含average_equivalent_weights
    const avgWeights = dataMapping.average_equivalent_weights;
    if (!avgWeights || !Array.isArray(avgWeights)) {
        console.warn('未找到average_equivalent_weights数据');
        return '数据格式不匹配，无法生成分析';
    }
    ...
    const featureNames = [
        ...
        'bbox_mds_1', 'bbox_mds_2', 'containment_mds', 'adjacency_mds',
        'bbox_width_n', 'bbox_height_n', 'bbox_fill_area'
    ];

    // 分析每个输出维度的主要特征
    avgWeights.forEach((dimensionWeights, j) => {
        analysis += `维度 Z_${j + 1}：\n`;
        
        // 找出最重要的输入特征（权重绝对值最大的两个）
        const weightEntries = dimensionWeights.map((w, i) => ({ 
            weight: w, 
            index: i,
        ...
        analysis += `主要由 `;
        topFeatures.forEach(({ weight, featureName }, i) => {
...
onMounted(async () => {
    ...
    try {
        // 并行获取六个数据源（包括矩阵数据）
        const [responseNormal, responseInit, responseMapping, responseEquivalentWeights, responseContainment, responseAdjacency] = await Promise.all([
            fetch(NORMAL_DATA_URL),
            fetch(INIT_DATA_URL),
            fetch(MAPPING_DATA_URL),
            fetch(EQUIVALENT_WEIGHTS_URL),
            fetch(CONTAINMENT_MATRIX_URL).catch(() => null), // 如果获取失败返回null
            fetch(ADJACENCY_MATRIX_URL).catch(() => null)    // 如果获取失败返回null
        ]);
        ...
        // 获取矩阵数据（可能不存在）
        if (responseContainment && responseContainment.ok) {
            containmentMatrixData = await responseContainment.json();
        }
        if (responseAdjacency && responseAdjacency.ok) {
            adjacencyMatrixData = await responseAdjacency.json();
        }
        ...
        // 定义4维输出维度名称
        const outputDimensions = ['z_1', 'z_2', 'z_3', 'z_4'];
        
        // 按顺序渲染各个组件
        await renderNormal(dataNormal, initialWidth, groupNames);
        await renderInit(dataInit, initialWidth, outputDimensions);
        await nextTick();
        ...
        // 渲染新特征的一维可视化
        renderContainmentAndAdjacencyVisualization(rawDataNormal, initialWidth);

        // 渲染矩阵可视化
        if (containmentMatrixData) {
            renderMatrix(containmentMatrixContainer.value, containmentMatrixData, '包含矩阵', 'Blues');
        }
        if (adjacencyMatrixData) {
            renderMatrix(adjacencyMatrixContainer.value, adjacencyMatrixData, '邻接矩阵', 'Reds');
        }
        ...
        const handleResize = debounce(async (entries) => {
            try {
                for (let entry of entries) {
                    ...
                    await renderInit(dataInit, newWidth, outputDimensions);
                    await nextTick();
                    if (xScaleNormal && xScaleInit) {
                        updateHighlights(selectedNodeIds.value);
                    }
                    ...
                    renderContainmentAndAdjacencyVisualization(rawDataNormal, newWidth);
                    
                    // 重新渲染矩阵以适应新的容器宽度
                    if (containmentMatrixData && containmentMatrixContainer.value) {
                        renderMatrix(containmentMatrixContainer.value, containmentMatrixData, '包含矩阵', 'Blues');
                    }
                    if (adjacencyMatrixData && adjacencyMatrixContainer.value) {
                        renderMatrix(adjacencyMatrixContainer.value, adjacencyMatrixData, '邻接矩阵', 'Reds');
                    }
                }
            } catch (error) {
    ...
    selectedIds.forEach(id => {
        ...
        if (xPos !== undefined) {
            svgNormal.append('rect')
                ...
                .attr('height', Math.max(0, normalChartContainer.value.clientHeight - marginNormal.top - marginNormal.bottom))
                .style('fill', 'none')
                .style('stroke', 'red')
    ...
    selectedIds.forEach(id => {
        ...
        if (xPos !== undefined) {
            svgInit.append('rect')
                ...
                .attr('height', Math.max(0, initChartContainer.value.clientHeight - marginInit.top - marginInit.bottom))
                .style('fill', 'none')
                .style('stroke', 'red')
                .style('stroke-width', '1.5px');
        }
...
const processDataNormal = (rawData) => {
    ...
    rawData.forEach((node) => {
        // 确保 node.features 长度为 22维
        if (node.features.length !== 22) {
            console.warn(`节点 ${node.id} 的特征数量为 ${node.features.length}，预期为 22`);
        }
        ...
        svgNormal.selectAll('.block')
            ...
            .on('mouseover', function (event, d) {
                tooltip.style('visibility', 'visible')
                    .text(`${groupNames[d.group]}: ${d.probability.toFixed(4)}`);
            })
    ...
    return new Promise((resolve) => {
        ...
        svgInit.selectAll('.block')
            ...
            .on('click', function (event, d) {
                ...
                // 获取点击的输出维度的权重
                const weightsForDimension = perSampleWeights[d.group]; // 长度为 22 的权重数组
...
const drawPerSampleLines = (weightsForDimension, clickedRectData, clickedRectNode) => {
    ...
    const normalData = normalCircles.nodes(); // 22 个输入节点
        ...
        perSampleLinesGroup.append('path')
            ...
            .on('mouseover', function(event) {
                ...
                d3.select(lineTooltip.value)
                    ...
                    .text(`w_${clickedRectData.group+1},${i+1} (${featureNames[i]}): ${w_j_i.toFixed(4)}`);
            })
            .on('mousemove', function(event) {
                const [mouseX, mouseY] = d3.pointer(event);
                d3.select(lineTooltip.value)
...
const drawLines = (dataMapping, containerWidth, groupNames) => {
    ...
    const avgWeights = dataMapping.average_equivalent_weights;
    if (!avgWeights || !Array.isArray(avgWeights)) {
        console.error('未找到average_equivalent_weights数据');
        return;
    }
    ...
    if (normalData.length !== inputDimensions.length || initData.length !== outputDimensions.length) {
        console.error('热力图节点数量与映射数据不匹配', {
            normalNodes: normalData.length,
            inputDimensions: inputDimensions.length,
            initNodes: initData.length,
            outputDimensions: outputDimensions.length
        });
        return;
    }
    ...
    // 为每个输出维度创建单独的线宽比例尺
    const lineWidthScales = avgWeights.map(outputWeight => {
    ...
    // 计算所有权重的范围，用于颜色映射
    const allWeights = avgWeights.flat();
    ...
    // 遍历每个输出维度
    avgWeights.forEach((outputWeight, j) => {
        outputWeight.forEach((w_j_i, i) => {
            ...
            svgLines.append('path')
                ...
                .on('mouseover', function(event) {
                    ...
                    d3.select(lineTooltip.value)
                        .style('visibility', 'visible')
                        .text(`w_${j+1},${i+1} (${inputDimensions[i]}): ${w_j_i.toFixed(4)}`);
                })
                .on('mousemove', function(event) {
                    const [mouseX, mouseY] = d3.pointer(event);
                    d3.select(lineTooltip.value)
...
// 添加 containment_mds 和 adjacency_mds 的专门可视化函数
const renderContainmentAndAdjacencyVisualization = (rawData, containerWidth) => {
    // 清空旧的图表
    d3.select('.containment-adjacency-container').remove();
    ...
    // 创建容器
    const container = d3.select(parentContainer.value)
        .append('div')
        .attr('class', 'containment-adjacency-container')
    ...
    container.append('h3')
        ...
        .style('color', '#333')
        .text('Containment & Adjacency MDS 特征分布');
    ...
    // 提取containment_mds和adjacency_mds数据
    const containmentData = rawData.map(d => ({
        id: d.id,
        value: d.features[17], // containment_mds 的索引
        type: 'containment'
    }));

    const adjacencyData = rawData.map(d => ({
        id: d.id,
        value: d.features[18], // adjacency_mds 的索引
        type: 'adjacency'
    }));

    // 创建比例尺
    const allValues = [...containmentData.map(d => d.value), ...adjacencyData.map(d => d.value)];
    const xScale = d3.scaleLinear()
    ...
    // 创建核密度估计器
    const kde = kernelDensityEstimator(kernelEpanechnikov(0.1), xScale.ticks(40));

    // 计算密度曲线
    const containmentDensity = kde(containmentData.map(d => d.value));
    const adjacencyDensity = kde(adjacencyData.map(d => d.value));

    // 创建Y比例尺
    const maxDensity = d3.max([...containmentDensity, ...adjacencyDensity], d => d[1]);
    ...
    // 添加containment散点
    svg.selectAll('.containment-point')
        .data(containmentData)
        .enter()
        .append('circle')
        .attr('class', d => `containment-point node-${d.id.split('/').pop()}`)
        .attr('cx', d => xScale(d.value))
        .attr('cy', height / 2 + 20)
        .attr('r', 3)
        .style('fill', d => selectedNodeIds.value.includes(d.id.split('/').pop()) ? '#ff6347' : '#2E86AB')
        .style('opacity', d => selectedNodeIds.value.includes(d.id.split('/').pop()) ? 1 : 0.6)
        .style('cursor', 'pointer')
        .on('mouseover', function(event, d) {
            ...
            d3.select(lineTooltip.value)
                .style('visibility', 'visible')
                .text(`${d.id.split('/').pop()} containment: ${d.value.toFixed(4)}`);
        })
        .on('mousemove', function(event) {
            const [mouseX, mouseY] = d3.pointer(event);
            d3.select(lineTooltip.value)
    ...
    // 添加adjacency散点
    svg.selectAll('.adjacency-point')
        .data(adjacencyData)
        .enter()
        .append('circle')
        .attr('class', d => `adjacency-point node-${d.id.split('/').pop()}`)
        .attr('cx', d => xScale(d.value))
        .attr('cy', height / 2 + 40)
        .attr('r', 3)
        .style('fill', d => selectedNodeIds.value.includes(d.id.split('/').pop()) ? '#ff6347' : '#A23B72')
        .style('opacity', d => selectedNodeIds.value.includes(d.id.split('/').pop()) ? 1 : 0.6)
        .style('cursor', 'pointer')
        .on('mouseover', function(event, d) {
            d3.select(this).attr('r', 5);
            d3.select(lineTooltip.value)
                .style('visibility', 'visible')
                .text(`${d.id.split('/').pop()} adjacency: ${d.value.toFixed(4)}`);
        })
        .on('mousemove', function(event) {
            const [mouseX, mouseY] = d3.pointer(event);
            d3.select(lineTooltip.value)
...
// 渲染矩阵可视化
const renderMatrix = (container, matrixData, title, colorScheme = 'Blues') => {
    if (!matrixData || !container) return;

    const margin = { top: 80, right: 50, bottom: 80, left: 80 };
    const matrixSize = matrixData.matrix.length;
    
    // 计算自适应的单元格大小
    const containerWidth = container.clientWidth - margin.left - margin.right - 30; // 减去padding
    const maxCellSize = Math.min(30, Math.max(15, containerWidth / matrixSize)); // 最小15px，最大30px
    const cellSize = maxCellSize;
    
    const width = matrixSize * cellSize;
    const height = matrixSize * cellSize;
    ...
    for (let i = 0; i < matrixSize; i++) {
        for (let j = 0; j < matrixSize; j++) {
            const cellValue = matrixData.matrix[i][j];
            const cell = g.append('g')
                .attr('class', `matrix-cell cell-${i}-${j}`)
                .attr('transform', `translate(${j * cellSize}, ${i * cellSize})`);
            ...
            cell.append('rect')
                .attr('width', cellSize)
                .attr('height', cellSize)
                .style('fill', colorScale(cellValue))
                .style('stroke', '#fff')
                .style('stroke-width', 1)
                .style('cursor', 'pointer');
            ...
            cell.on('mouseover', function(event) {
                ...
                d3.select(lineTooltip.value)
                    .style('visibility', 'visible')
                    .text(`${matrixData.element_ids[i]} → ${matrixData.element_ids[j]}: ${cellValue}`);
            })
            .on('mousemove', function(event) {
            ...
            .on('click', function() {
                // 点击时选中对应的节点
                const sourceId = matrixData.element_ids[i];
                const targetId = matrixData.element_ids[j];
                
                if (!selectedNodeIds.value.includes(sourceId)) {
                    store.commit('ADD_SELECTED_NODE', sourceId);
                }
                if (!selectedNodeIds.value.includes(targetId)) {
                    store.commit('ADD_SELECTED_NODE', targetId);
                }
            });
        }
    }
    ...
    for (let i = 0; i < matrixSize; i++) {
        g.append('text')
            .attr('class', `row-label row-label-${i}`)
            .attr('x', -10)
            .attr('y', i * cellSize + cellSize / 2)
            .attr('dy', '0.35em')
            .attr('text-anchor', 'end')
            ...
            .text(matrixData.element_ids[i]);
    }

    // 添加列标签
    for (let j = 0; j < matrixSize; j++) {
        g.append('text')
            .attr('class', `col-label col-label-${j}`)
            .attr('x', j * cellSize + cellSize / 2)
            .attr('y', -10)
            .attr('text-anchor', 'middle')
            .style('font-size', '12px')
            .style('font-family', 'Arial, sans-serif')
            .text(matrixData.element_ids[j])
            .attr('transform', `rotate(-45, ${j * cellSize + cellSize / 2}, -10)`);
    }
};

// 更新矩阵高亮显示
const updateMatrixHighlight = (container, matrixData, selectedIds) => {
    if (!matrixData || !container) return;

    const matrixSize = matrixData.matrix.length;
    
    // 重置所有单元格和标签的样式
    d3.select(container).selectAll('.matrix-cell rect')
        .style('opacity', 0.7);
    
    d3.select(container).selectAll('.row-label, .col-label')
        .style('font-weight', 'normal')
        .style('fill', '#333');

    // 高亮选中节点对应的行和列
    selectedIds.forEach(nodeId => {
        const index = matrixData.element_ids.indexOf(nodeId);
        if (index !== -1) {
            // 高亮整行
            d3.select(container).selectAll(`.cell-${index}-${Array.from({length: matrixSize}, (_, i) => i).join(`, .cell-${index}-`)}`)
                .select('rect')
                .style('opacity', 1);
            
            // 高亮整列
            for (let i = 0; i < matrixSize; i++) {
                d3.select(container).select(`.cell-${i}-${index} rect`)
                    .style('opacity', 1);
            }
            
            // 高亮对应的标签
            d3.select(container).select(`.row-label-${index}`)
                .style('font-weight', 'bold')
                .style('fill', '#ff6347');
            
            d3.select(container).select(`.col-label-${index}`)
                .style('font-weight', 'bold')
                .style('fill', '#ff6347');
        }
    });
...
watch(selectedNodeIds, (newSelectedIds) => {
    ...
    // 更新 containment 和 adjacency 散点
    d3.selectAll('.containment-point')
        .style('fill', d => newSelectedIds.includes(d.id.split('/').pop()) ? '#ff6347' : '#2E86AB')
        .style('opacity', d => newSelectedIds.includes(d.id.split('/').pop()) ? 1 : 0.6);

    d3.selectAll('.adjacency-point')
        .style('fill', d => newSelectedIds.includes(d.id.split('/').pop()) ? '#ff6347' : '#A23B72')
        .style('opacity', d => newSelectedIds.includes(d.id.split('/').pop()) ? 1 : 0.6);

    // 更新矩阵高亮显示
    if (containmentMatrixData) {
        updateMatrixHighlight(containmentMatrixContainer.value, containmentMatrixData, newSelectedIds);
    }
    if (adjacencyMatrixData) {
        updateMatrixHighlight(adjacencyMatrixContainer.value, adjacencyMatrixData, newSelectedIds);
    }
}, { deep: true });
