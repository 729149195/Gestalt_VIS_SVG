...
const featureNameMap = {
  ...
  'containment_mds': 'containment relation',
  'adjacency_mds': 'adjacency relation',
  'bbox_width_n': 'width',
...
const featureRanges = {
  ...
  'containment_mds': { q1: 0.0, q3: 1.0 },
  'adjacency_mds': { q1: 0.0, q3: 1.0 },
...
const emit = defineEmits(['scroll', 'drawer-state-change'])
...
const globalMouseOverHandler = (event) => {
  ...
  if (drawerElement && drawerElement.style.display !== 'none') return;
...
// 添加延迟生成建议的定时器引用
let suggestionGenerationTimer = null;

// 修改监听逻辑，当选中节点变化时延迟1秒再重新获取数据
watch(() => store.state.selectedNodes.nodeIds, () => {
  // 清除之前的定时器
  if (suggestionGenerationTimer) {
    clearTimeout(suggestionGenerationTimer);
  }
  
  // 设置新的定时器，1秒后执行
  suggestionGenerationTimer = setTimeout(() => {
    fetchDataAndGenerateAnalysis();
  }, 1000);
}, { deep: true, immediate: true });

// 添加对 visualSalience 的监听，当它变化时延迟1秒再重新生成分析内容
...
const isMdsFeature = (featureKey) => {
  return featureKey === 'bbox_mds_1' || featureKey === 'bbox_mds_2' || 
         featureKey === 'containment_mds' || featureKey === 'adjacency_mds';
};
