                ...
                <div v-if="file" class="data-cards">
                    <div class="maxtistic">
                        ...
                        <div class="analysis-section" :style="{ height: analysisHeight + '%' }">
                            <analysisWords title="Feature dimension mapping analysis" :update-key="componentKey2" class="analysis-words" @drawer-state-change="handleDrawerStateChange" />
                        </div>
                    </div>
                </div>
            </div>
...
const handleDrawerStateChange = (isOpen) => {
    ...
    // 当抽屉打开时，强制更新 maxstic 组件以确保显示最新数据
    if (isOpen) {
