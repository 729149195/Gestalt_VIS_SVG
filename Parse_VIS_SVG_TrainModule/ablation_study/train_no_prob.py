...
def save_model(args, model, optimizer, scheduler, current_epoch):
    out = os.path.join(args.model_path, f"ablation_no_prob_model.tar")
    state = {
        ...
        for filename in os.listdir(data_dir):
            if filename.endswith('.json'):
                ...
                with open(filepath, 'r') as f:
                    data = json.load(f)
                    all_features = [torch.tensor(feature, dtype=torch.float32) 
                    ...
                    for group in groups:
                        ...
                        group_features = [torch.tensor(feature, dtype=torch.float32) 
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='消融实验：无概率选取版本的对比学习训练')
    ...
    # 模型加载
    if args.reload:
        model_fp = os.path.join(args.model_path, "ablation_no_prob_model.tar")
