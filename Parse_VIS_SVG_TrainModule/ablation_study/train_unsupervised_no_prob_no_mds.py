...
def save_model(args, model, optimizer, scheduler, current_epoch):
    out = os.path.join(args.model_path, f"ablation_unsupervised_no_prob_no_mds_model.tar")
    state = {
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='消融实验：基础无监督版本的对比学习训练')
    ...
    # 模型加载
    if args.reload:
        model_fp = os.path.join(args.model_path, "ablation_unsupervised_no_prob_no_mds_model.tar")
