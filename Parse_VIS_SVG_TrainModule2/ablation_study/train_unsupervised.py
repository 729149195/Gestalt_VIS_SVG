...
这个版本是无监督学习版本，不使用标注数据，但保留概率选取和MDS特征。
主要修改：
1. 移除对标注数据(groups)的依赖
2. 使用all_features中的所有特征进行无监督学习
3. 基于距离的概率选取应用于所有样本对
4. 保持MDS特征
"""

def save_model(args, model, optimizer, scheduler, current_epoch):
    out = os.path.join(args.model_path, f"ablation_unsupervised_model.tar")
    state = {
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='消融实验：无监督版本的对比学习训练')
    ...
    # 模型加载
    if args.reload:
        model_fp = os.path.join(args.model_path, "ablation_unsupervised_model.tar")
