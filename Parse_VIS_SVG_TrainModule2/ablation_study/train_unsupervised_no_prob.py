...
def save_model(args, model, optimizer, scheduler, current_epoch):
    out = os.path.join(args.model_path, f"ablation_unsupervised_no_prob_model.tar")
    state = {
...
class UnsupervisedFeaturePairDataset(Dataset):
    def __init__(self, data_dir, distance_threshold=0.5):
        self.sample_pairs = []
    ...
    def sample_pairs_for_feature(self, base_feature, all_features, chosen_set):
        ...
        if pos_candidates:
            ...
            selected_indices = np.random.choice(len(pos_candidates), 
        ...
        if neg_candidates:
            ...
            selected_indices = np.random.choice(len(neg_candidates), 
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='消融实验：无监督+MDS特征但无概率选取版本的对比学习训练')
    ...
    # 模型加载
    if args.reload:
        model_fp = os.path.join(args.model_path, "ablation_unsupervised_no_prob_model.tar")
